...
- リポジトリの**実体ディレクトリ `~/dev/shibasu-keiro` は「main 統合＋デプロイ専用」**。
...
git fetch origin
git worktree add ../shibasu-keiro-<topic> -b feat/<topic> origin/main
# 起点ズレ確認（0 であること）
git -C ../shibasu-keiro-<topic> rev-list --count origin/main..HEAD

# 一覧 / 後片付け
git worktree list
git worktree remove ../shibasu-keiro-<topic>   # マージ後に撤去
```

- 命名規則: `feat/<topic>` `fix/<topic>` `chore/<topic>`。worktree dir は `../shibasu-keiro-<topic>`。
- **同じファイルを2つの worktree で同時編集しない**（役割境界を守る）。
- worktree は本体 repo の `node_modules` を symlink すると検証が速い（`ln -s ../shibasu-keiro/node_modules node_modules`）。
